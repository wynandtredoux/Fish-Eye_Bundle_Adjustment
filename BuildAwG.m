...
for i=1:length(PHO)   
    ...
    xhat_index = u_perimage*numimg + cam_num*u_percam-(u_percam-1);
    xhat_count = 0;
    
    if Estimate_xp == 1
        xp = xhat(xhat_index+xhat_count);
        xhat_count = xhat_count + 1;
    else    
        xp = INT{int_index + 1,1};
    end
    if Estimate_yp == 1
        yp = xhat(xhat_index+xhat_count);
        xhat_count = xhat_count + 1;
    else    
        yp = INT{int_index + 1,2};
    end
    if Estimate_c == 1
        c = xhat(xhat_index+xhat_count);
        %xhat_count = xhat_count + 1;
    else    
        c = INT{int_index + 1,3};
    end    
