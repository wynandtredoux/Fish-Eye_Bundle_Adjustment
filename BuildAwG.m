...
% Data:
% PHO - photo measurements [Point  image  x  y]
% EXT - Image Exterior Orientation [Image  camera  Xc  Yc  Zc  omega  phi  kappa]
% CNT - Object Point Coordinates [PointID  X  Y  Z]
% INT - Camera interior orientations [Camera  y_axis_dir  x_min  y_min  x_max  y_max]
%                                    [xp  yp  c  k1...kn  p1  p2]
% TIE - List of tie points [PointID] (can be empty if there are no tie points)
% xhat - estimated unknowns [Unknown_value]
% Options:
...
function [error, A, misclosure, G, dist_scaling] = BuildAwG(PHO,EXT,CNT,INT,TIE,xhat,... % data
    BuildG, Estimate_Xc, Estimate_Yc, Estimate_Zc, Estimate_w, Estimate_p, Estimate_k, Estimate_xp, Estimate_yp, Estimate_c, Estimate_radial, Num_Radial_Distortions, Estimate_decent, type) % settings
%% Setup
error = 0;
...
for i=1:length(PHO)   
    ...
    elseif strcmp(type,'pinhole')
        ...
        errordlg('BuildAwG, invalid type');
        error = 1;
        return
    end
